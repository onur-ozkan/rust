...
mod defaults {
    ...
    fn build_cross_compile() {
        ...
        assert_eq!(first(cache.all::<compile::Std>()), &[
            ...
            std!(TEST_TRIPLE_2 => TEST_TRIPLE_2, stage = 1),
        ]);
        ...
        assert_eq!(first(cache.all::<tool::Rustdoc>()), &[
            tool::Rustdoc { compiler: Compiler::new(0, a).downgraded_from(1) },
            tool::Rustdoc { compiler: Compiler::new(0, b).downgraded_from(1) },
        ],);
        assert_eq!(first(cache.all::<compile::Rustc>()), &[
            rustc!(TEST_TRIPLE_1 => TEST_TRIPLE_1, stage = 0),
...
mod dist {
    ...
    fn dist_baseline() {
        ...
        assert_eq!(first(cache.all::<dist::Rustc>()), &[dist::Rustc {
            compiler: Compiler::new(2, a)
        },]);
        assert_eq!(first(cache.all::<dist::Std>()), &[dist::Std {
            compiler: Compiler::new(1, a),
            target: a
        },]);
    ...
    fn build_all() {
        ...
        assert_eq!(builder.cache.all::<compile::Assemble>().len(), 6);
